    ...
    (cond
      ...
      :else
        (let [new-c (base/add (:coeff x) ['polynomial c])]
          (if (base/alg-zero? new-c) () (list (make-term 0 new-c)))))))
...
(defn zero? [t]
  (every? (comp base/alg-zero? :coeff) t))

(defn strip-zeros [t]
  (cond
    (empty? t)
      ()
    (base/alg-zero? (:coeff (clojure.core/first t)))
      (strip-zeros (rest t))
    :else
